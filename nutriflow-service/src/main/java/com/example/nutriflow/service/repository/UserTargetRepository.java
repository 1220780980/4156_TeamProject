...
public interface UserTargetRepository extends JpaRepository<UserTarget, Integer> {
     ...
     * @return Optional containing the user's most recent target if found, empty otherwise
     */
    @Query("SELECT ut FROM UserTarget ut WHERE ut.userId = :userId ORDER BY ut.createdAt DESC LIMIT 1")
    Optional<UserTarget> findLatestByUserId(@Param("userId") Integer userId);
}