...
public class UserTargetService {
     ...
     * @return Optional containing the user's latest targets if found, empty otherwise
     */
    public Optional<UserTarget> getUserTargets(Integer userId) {
     ...
     * If targets exist, they will be updated; otherwise, new targets will be created
     * 
     * @param userId the ID of the user
     * @param request the request containing updated target information
     * @return Optional containing the updated/created targets if successful, empty if user not found
     */
    @Transactional
    public Optional<UserTarget> updateUserTargets(Integer userId, UpdateUserTargetRequestDTO request) {
        // First verify the user exists
        if (!userRepository.existsById(userId)) {
            return Optional.empty();
        }
        ...
        Optional<UserTarget> existingTargetOpt = userTargetRepository.findLatestByUserId(userId);
     ...
     */
    private void updateTargetFields(UserTarget target, UpdateUserTargetRequestDTO request) {
